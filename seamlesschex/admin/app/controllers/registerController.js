(function() {
	...
	function RegisterController($scope, $auth, $state, $http, $rootScope,$stateParams,  API_URL, $payments, $timeout) {
		...
		vm.register = function() {
			...
			$scope.error_message_all_field = false;
			...
			if(angular.isUndefined(vm.user.plan) || angular.isUndefined(vm.user.number) ||
			 angular.isUndefined(vm.user.expiration) || angular.isUndefined(vm.user.cvc) || angular.isUndefined(vm.user.agree)) {
				$scope.error_message_all_field ='All Field Required!';
				return false;
       		}
			...
			if(vm.user.agree == 'yes'){
				...
				vm.registerError = true;
				vm.registerErrorText = 'Please agree terms and condition';
				$scope.isDisabled = false;
			}
		}
		...
		$scope.hideErrorMessage = function () {
			...
			$scope.error_message_name     = false;
			$scope.error_message_cname    = false;
			$scope.error_message_email    = false;
			$scope.error_message_website  = false;
			$scope.error_message_saddress = false;
			
			$scope.error_message_city     = false;
			$scope.error_message_state    = false;
			$scope.error_message_zip      = false;
			$scope.error_message_phone    = false;
			$scope.error_message_business_type = false;
			$scope.error_message_password  = false;
			$scope.error_message_privacypolicy = false;
		 ...
		 $scope.incrementStep = function() {
			...
			if(angular.isUndefined(vm.user.name)) {
       		...
       		if(angular.isUndefined(vm.user.cname)){
				$scope.error_message_cname = 'Enter First Name!';
				return false;
			}else if(vm.user.cname!=''){
				var cname = vm.user.cname;
			...
			if(angular.isUndefined(vm.user.state)){
				$scope.error_message_state = "Enter State!";
				return false;
			}
		
			if(angular.isUndefined(vm.user.zip)){
				$scope.error_message_zip = "Enter Valid Zip!";
				return false;
			}
			if(angular.isUndefined(vm.user.phone)){
				$scope.error_message_phone = "Enter Phone number!";
				return false;
			}
