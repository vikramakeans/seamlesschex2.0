		...
		vm.login = function() {
			...
			$auth.login(credentials).then(function() {
				...
				return $http.get( API_URL + 'api/authenticate/user').then(function(response) {
					...
					// For company admin and status = 0, for step_users table, if user not completed need to redirect to step2 form
					if($rootScope.currentUser.role == 3 && $rootScope.currentUser.status == 0){
						//$rootScope.authcompAdmin = true;
						$state.go('upgrade-account', { sc_token: $rootScope.currentUser.sc_token});
					}
					// For company admin
